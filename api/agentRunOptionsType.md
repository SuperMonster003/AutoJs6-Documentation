...
| confirm | `'default'` \| `'cautious'` | 沿用全局设置与预设, 初始为 default; cautious 对普通变更也要求确认, 不能放宽任一层的 cautious |
...
表中是未配置全局预算时的初始值. AI Agent 构建号 60 起, 设置页可在协议硬上限内调整它们, 留空表示沿用初始值. 有效预算取全局设置, 任务归属对应的时长上限, 宿主 token 授权和预设预算的交集. 单次任务覆盖只能在有效预算内继续收紧. 例如全局 maxSteps 为 100, 预设为 80 时, 任务传入 maxSteps: 81 会被拒绝; 全局为 5, 预设为 80 时仍只能使用 5 步.

全局 maxDurationMs 可以设置为 3600000, 但普通任务仍最多 1800000 毫秒, detached 最多 3600000 毫秒. 全局时长留空时, 普通任务仍默认为 600000 毫秒, detached 为 1800000 毫秒. 预设只能缩小当前全局预算, 不能单独提高它.
