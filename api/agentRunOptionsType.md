...
| 选项 | 类型 | 默认与含义 |
| --- | --- | --- |
| preset | [string](dataTypes#string) | 当前仅 default |
| target | [string](dataTypes#string) | 完整模型目标 ID, 省略使用插件默认目标 |
| tools | [string](dataTypes#string)[[]](dataTypes#array) 或 [object](dataTypes#object) | 工具组数组, 或含 enable/disable 数组的对象 |
| budget | [object](dataTypes#object) | 任务预算覆盖 |
| confirm | `'default'` \| `'cautious'` | default; cautious 对普通变更也逐次确认 |
...
| memory | [boolean](dataTypes#boolean) | true; false 不注入偏好记忆 |
| scriptRoots | [string](dataTypes#string)[[]](dataTypes#array) | 附加脚本根选择, 最多 32 项, 只能缩小已批准范围 |
...
context 与 parameters 合并注入模型时总计仍不能超过 8 KiB. 预填参数不授权任何文件访问或设备操作. scriptRoots 必须为绝对目录, 不含父目录跳转或隐藏越界路径; 宿主再次进行真实路径校验.
...
当前插件开发版本只允许在默认预算内收紧. 宿主同时检查协议硬上限: 200 步, 300 次调用, 普通任务 30 分钟 / detached 60 分钟, 1000000 token. 通过宿主硬上限校验并不表示插件会接受大于当前默认值的预算; 插件拒绝会返回失败句柄. 所有预算值必须为正整数.
