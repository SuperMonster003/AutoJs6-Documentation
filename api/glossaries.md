...
本文档中的 "内置模块" 泛指 AutoJs6 向脚本运行环境提供的模块对象或可加载模块.

### 查看内置模块源代码

自 AutoJs6 6.6.0 起, 主要脚本 API 的实现位于
[`runtime/api/augment`](http://project.autojs6.com/tree/master/app/src/main/java/org/autojs/autojs/runtime/api/augment).
可复用的 JavaScript 模块仍位于
[`assets/modules`](http://project.autojs6.com/tree/master/app/src/main/assets/modules),
例如 CommonJS 加载器, `i18n`, `events` 和若干 Node.js 兼容模块.

APK 中的 `assets/modules` 只包含后一类 JavaScript 文件, 因此不能再通过解压该目录查看全部内置 API 的实现.

## 编译器

语法编译器是一个能够逐行读取代码的程序.<br>
它了解代码如何匹配编程语言所定义的语法, 以及代码应该做什么.

## JavaScript 引擎

JavaScript 引擎是一个计算机程序.<br>
...
正则表达式字面量在 JavaScript 中用一对 `/` 符号表示,<br>
...
Truthy (真值) 指 `Boolean(truthy)` 返回 `true` 的值.<br>
...
又称 [ 原生对象 / 标准内置对象 ], 内置对象与宿主无关, 是独立于宿主环境的 ECMAScript 实现提供的对象.<br>
它们在 ECMAScript 程序开始执行前就存在, 本身就是实例化内置对象, 开发者无需再去实例化.<br>
...
上述扩展均为自定义扩展, 它们实现了自定义的属性或方法, 往往是针对个人项目使用的.<br>
而针对 ECMAScript 规范中的新功能的扩展, 则被称为 [填泥 (Polyfill)](#polyfill).

> 注: 扩展内置对象往往是 **危险** 的.
>
> 扩展 JavaScript 原生对象意味着将属性或方法添加到其原型或内置对象上,<br>
> 其潜在的危险包括但不限于以下几种情况:
>
...
进行直接扩展后, 所有扩展属性和方法会按需附加到内置对象或其原型上, 详情参考每个扩展对象的小节内容.<br>
直接扩展是不安全的, 需谨慎使用.
...
又称 [ 代码填泥 / 填泥 / 腻子 / 泥子 ], 是一个完整的代码块, 用于为不支持原生 ECMAScript 新功能的环境提供功能支持.<br>
详见 [代码填泥](polyfill) 章节.

## Shim

又称 [ 代码垫片 / 垫片 / 填隙片 ], 是一种小型函数库, 可以用来截取 API 调用或修改传入参数, 最后自行处理对应操作或者将操作交由其它地方执行.<br>
垫片可以在新环境中支持老 API, 也可以在老环境里支持新 API.<br>
...
通常, 应用资源与代码是分离的, 以便于独立维护.<br>
资源可进行分组并放入专门命名的资源目录中, 如 [ animator / color / drawable / layout / values / menu ] 等.<br>
...
信息集控件包含一系列子控件, 它们类似 HTML 表格那样按行列方式分布.<br>
例如垂直列表是一个信息集控件, 它有一列多行作为子控件; 表格也是一个信息集控件, 它有多列多行作为子控件.<br>
...
`Rect(10.9, 12.6 - 120.37, 1882.02)` 外展后得到<br>
`Rect(10, 12, 121, 1883)`
...
`Rect(10.9, 12.6 - 120.37, 1882.02)` 内收后得到<br>
`Rect(11, 13, 120, 1882)`
...
`阈值 0` 等效于 `相似度 1` (完全匹配, 不允许丝毫误差)<br>
`阈值 4` 约等效于 `相似度 0.984` (匹配时可以容忍一点误差)<br>
`阈值 128` 等效于 `相似度 0.5` (匹配时误差容忍相对宽松)<br>
...
- [`images.threshold(image, threshold, maxVal, type?)`](image#m-images-threshold-image-threshold-maxval-type)

[`images.adaptiveThreshold(image, maxValue, adaptiveMethod, thresholdType, blockSize, C)`](image#m-images-adaptivethreshold-image-maxvalue-adaptivemethod-thresholdtype-blocksize-c)
不接收固定阈值. 它根据像素邻域计算局部阈值, 再使用参数 **C** 调整计算结果.
...
国际单位制规定亮度的符号是 `Lv`, 单位为 `坎德拉每平方米 (cd/m²)`, 另一个常用非国际单位为 `尼特 (nit)`.<br>
亮度是一个物理量, 它可被测量及计算.

[Lightness](#lightness) 和 [Brightness](#brightness) 用于表示人眼对光亮的实际感受.<br>
...
视亮度 (Brightness) 是对于光源或物体表面明暗的视知觉特性, 是一个感知量.<br>
视亮度是视觉的特性, 对视觉目标的辐射或反射的光亮度的感知.<br>
