...
`project.json` 中的 `launchConfig.requiresSharedStorage` 是布尔值, 默认 `false`. 打包界面的 "启动需要共享存储访问" 选项与此字段对应, 可随打包配置保存.
...
此选项适用于启动后需要直接读写 `/sdcard/Download` 等共享路径的项目. 打包资源会解包到应用私有目录, 仅使用这些资源或应用专属文件的脚本无需开启. 通过系统文件选择器或 MediaStore 访问文件的项目, 应按相应接口的授权规则处理, 无需仅为这些访问开启此选项. 打包器不会分析脚本代码来推断文件访问需求.

| 配置或状态 | 启动行为 |
| --- | --- |
| `requiresSharedStorage: false` 或未配置 | 不在启动时申请存储权限, 直接执行脚本. 运行中的共享路径访问仍受 Android 权限限制 |
| `requiresSharedStorage: true`, 已有可用授权 | 执行脚本 |
| `requiresSharedStorage: true`, 缺少授权 | 显示存储说明并暂停启动; 取消或拒绝授权不会执行脚本 |
| 从系统权限设置返回 | 重新检查实际权限; 返回设置页本身不代表授权成功 |
| 缺少 Manifest 权限声明 | 提示作者重新打包并包含所需权限 |

Android 11 及以上使用 "所有文件访问" 设置. Android 10 及以下申请旧存储运行时权限, 无法再次弹出授权框时进入应用设置. 旧安装仍具有可用的传统存储访问权限时, 可直接继续启动. "所有文件访问" 不允许访问其他应用的私有目录, 也不保证可访问不可用的卷或任意受限路径.

如果自定义了打包权限列表, 跨系统支持需要保留 `READ_EXTERNAL_STORAGE`, `WRITE_EXTERNAL_STORAGE` 和 `MANAGE_EXTERNAL_STORAGE` 对应的 Android 权限声明. 勾选启动选项不会自动修改权限列表. 更新旧项目后, 作者应按其实际共享路径需求设置此字段, 并验证原有文件能否访问.

同时关闭桌面入口时, 缺少必要存储权限期间会保留桌面入口供用户恢复. 授权后重新启动会应用原有隐藏设置; 也可从打包应用设置页点击 "运行脚本". 开机入口缺少权限时记录停止原因, 保留恢复入口, 不自动弹出授权页面. 存储授权不会豁免开机启动, 后台服务或音频播放的其他系统限制.
