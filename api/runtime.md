...
在 Android 17 (API 37) 及以上系统中, 如果当前应用的 targetSdk 至少为 37, 访问局域网需要 `access_local_network` 权限. AutoJs6 在实际 targetSdk 至少为 37 时声明该权限, 也可从抽屉或设置页申请. 旧系统或 targetSdk 不足 37 时会跳过此权限请求.

```js
runtime.requestPermissions(["access_local_network"]);
```

此调用不会等待授权完成. 应在用户主动操作时发起请求, 等用户完成授权后再重试网络操作, 不要在下一行立即假定已授权. 后台脚本启动权限界面仍受 Android 后台活动启动规则限制.

权限涉及局域网 TCP 出站连接与入站连接, UDP 和 WebSocket 等. 拒绝时 TCP 可能表现为超时, UDP 可能返回权限错误. AutoJs6 的 HTTP, WebSocket 和 Node 受控网络桥会在相关连接失败且未授权时补充恢复提示, 保留原始错误; 提示不代表已经排除了地址, 服务端, TLS 或防火墙问题. 直接使用 Java Socket 或第三方 MQTT 客户端的脚本仍收到对应库的异常, 可使用同一权限申请入口恢复. 网络调用不会自动弹出授权界面.

同一用户资料下的 `127.0.0.1` 和 `[::1]` 回环通信可用于本机服务, 但不能据此保证跨工作资料的回环可达. 插件进程拥有各自的应用权限; 宿主授权不会代替插件授权.

> 参阅: [Android 运行时权限](https://developer.android.com/training/permissions/requesting), [本地网络权限](https://developer.android.com/privacy-and-security/local-network-permission)
